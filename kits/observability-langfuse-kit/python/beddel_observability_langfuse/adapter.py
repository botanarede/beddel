...
class LangfuseTracerAdapter(ITracer[LangfuseSpan]):
    ...
    def log_event(self, name: str, metadata: dict[str, Any] | None = None) -> None:
        ...
        Creates a standalone trace with the given *name* and *metadata*.
        Used by primitives (e.g. ``DecidePrimitive``) to record structured
        events without requiring an active span.

        Args:
            name: Event name (e.g. ``"decision"``).
        ...
        try:
            ...
            _logger.warning(
                "Failed to log Langfuse event %r — ignoring",
                name,
                exc_info=True,
            )
