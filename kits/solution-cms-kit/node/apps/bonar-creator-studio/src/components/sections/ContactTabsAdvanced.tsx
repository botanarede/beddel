...
function ContactInfoTab({ siteConfig }: { siteConfig: SiteConfig }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const messageData = {
        subject: `Contato - ${siteConfig.name}`,
        text: `Nome: ${name}\nEmail: ${email}\nMensagem: ${message}`,
...
function BirthdayTab({ siteConfig }: { siteConfig: SiteConfig }) {
  const instagramContactUrl = siteConfig.social?.instagram || '#'

  return (
    <div className="max-w-4xl mx-auto text-center">
      <h2 className="mb-4 text-2xl font-bold text-green-800">Lista de Aniversário</h2>
      <p className="mb-8 text-gray-600">
        Comemore seu aniversário conosco! Entre em contato pelo Instagram para fazer sua reserva.
      </p>
      <a
...
function CorporateEventsTab({ siteConfig }: { siteConfig: SiteConfig }) {
  const { setItem } = useDynamicTable()
  const [isLoading, setIsLoading] = useState(false)
  const [sent, setSent] = useState(false)
    ...
    try {
      ...
      await setItem(
        'emails',
        {
          ...
          from: `${siteConfig.name} <${siteConfig.email}>`,
          message: {
            subject: `Evento Corporativo - ${siteConfig.name}`,
            text,
            html: text.replace(/\n/g, '<br/>'),
          },
          createdAt,
...
interface ContactTabsAdvancedProps {
  defaultTab?: string
...
/**
 * Connected ContactTabs with advanced features (multi-tab contact forms).
 * Provides 4 tabs with tenant-agnostic content driven by siteConfig props.
 */
export function ContactTabsAdvanced({ defaultTab = 'contact', siteConfig, ...rest }: ContactTabsAdvancedProps) {
  ...
  const tabs: TabDefinition[] = [
    ...
    },
    {
      id: 'birthday',
      label: 'Aniversário',
      content: <BirthdayTab siteConfig={siteConfig} />,
    },
    {
      id: 'corporate',
      label: 'Eventos Corporativos',
      content: <CorporateEventsTab siteConfig={siteConfig} />,
    },
    {
      id: 'faq',
