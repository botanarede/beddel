...
from beddel_solution_cms._errors import CMS_BUILD_FAILED, CMSError
from beddel_solution_cms.tools.build import build_site
...
class TestBuildSiteSuccess:
    ...
    def test_successful_build_returns_success_dict(
        ...
        with (
            patch(
                "beddel_solution_cms.tools.build.SafeSubprocessRunner.run",
                return_value=_success_result(stdout="Route / 200\nBuild complete"),
            ),
            patch(
                "beddel_solution_cms.tools.build.get_kit_root",
...
class TestErrorCode:
    ...
    def test_cms_build_failed_resolves(self) -> None:
        ...
        from beddel_solution_cms._errors import CMS_BUILD_FAILED as imported
