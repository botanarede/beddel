...
(``repo/kits/solution-cms-kit/``), never an external monorepo. This module has
...
def get_kit_root() -> Path:
    ...
    This file lives at ``python/beddel_solution_cms/tenant_context.py``, so the
    kit root is 3 parents up.

    Returns:
        Absolute path to ``repo/kits/solution-cms-kit/``.
    """
