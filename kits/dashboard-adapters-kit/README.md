# dashboard-adapters-kit

Dashboard pipeline adapters normalizing agent events to Pipeline Protocol.

## Language targets

| Language    | Status | Notes |
| ----------- | ------ | ----- |
| Python      | `planned` | Planned — implementation_path: `./python/src/beddel_dashboard_adapters/__init__.py` |
| TypeScript  | `implemented` | Package: `@beddel/dashboard-adapters` |

## Layout

- `python/README.md` — placeholder describing the planned implementation
- `typescript/src/` — TypeScript source + `package.json`

## Manifest

See [`kit.yaml`](./kit.yaml) for the full contract surface (tools, adapters, contracts, workflows) and language-specific declarations.

## See also

- [`spec/PROTOCOL.md`](../../spec/PROTOCOL.md) — kit protocol version and field semantics
- [`CONTRIBUTING.md`](../../CONTRIBUTING.md) — how to contribute kits and promote planned targets to implemented
